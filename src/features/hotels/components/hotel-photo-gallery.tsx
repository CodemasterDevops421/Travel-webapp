...
export function HotelPhotoGallery({ photos, hotelName, lightboxIndex, onOpen, onClose }: HotelPhotoGalleryProps) {
  ...
  return (
    <>
      <section className="grid h-[400px] gap-2 overflow-hidden rounded-[32px] md:h-[520px] md:grid-cols-[2fr,1fr]">
        {photos[0] ? (
          <button type="button" className="group relative h-full w-full overflow-hidden rounded-[28px] bg-muted" onClick={() => onOpen(0)}>
          ...
          <article className="flex h-full w-full items-center justify-center rounded-[28px] bg-muted">
        ...
        <div className="grid h-full grid-cols-2 grid-rows-2 gap-2">
          {photos.slice(1, 5).map((photo, index) => {
            ...
            <button
              key={`${photo}-${index}`}
              type="button"
              className="group relative h-full w-full overflow-hidden rounded-[24px] bg-muted"
